...
原生支持所有能够保存值的 C++11 基础类型、固定宽度整数别名和字符串，也可以
通过 `flag_traits<T>` 扩展自定义参数类型。

## 特性

- 仅需包含 `cflag.h`，无需编译或链接二进制库。
- 使用 `var<T>` 和 `varp<T>` 进行类型安全的参数注册。
- 原生支持全部字符类型、有符号整数、无符号整数和浮点类型，以及 `bool`、
...
| `char`、`signed char`、`unsigned char` | 对应的类型名 | 目标类型范围内的数值代码单元 |
| `wchar_t`、`char16_t`、`char32_t` | 对应的类型名 | 目标类型范围内的数值代码单元 |
| `char8_t`（C++20） | `char8_t` | 目标类型范围内的数值代码单元 |
| `short`、`int`、`long`、`long long` | 对应的类型名 | 目标类型有符号范围内的十进制整数 |
| 上述类型的无符号版本 | 对应的类型名 | 从 `0` 到目标类型最大值的十进制整数 |
| `<cstdint>` 中的固定宽度整数别名 | 底层类型名或固定宽度类型名 | 精确目标范围内的十进制整数 |
| `float`、`double`、`long double` | 对应的类型名 | 可被 `std::stof`、`std::stod` 或 `std::stold` 完整解析的值 |
| `std::nullptr_t` | `nullptr` | 严格匹配的文本 `nullptr` |
| `std::string` | `string` | 任意字符串，包括显式传入的空字符串 |

字符类型使用数值代码单元，不会把参数文本解码成字符或 UTF 编码。例如，
`--letter=65` 会把代码单元值 65 保存到 `char` 变量。

声明固定宽度整数变量时需要包含 `<cstdint>`。固定宽度类型是基础整数类型的
别名，因此 C++ 不一定能保留注册时使用的类型拼写；帮助信息可能显示底层类型
名，也可能显示固定宽度名称。解析范围始终以目标类型的
`std::numeric_limits` 为准。

`void` 是唯一无法注册的 C++11 基础类型：C++ 不允许创建 `void` 类型的对象，
因此也无法提供对应的参数目标变量。
