...
// Reads server flags from a gflags-style file with cflag::parse_file.
//
// The file path may be given as the first positional argument; otherwise the
// bundled example/flags.gflags is used. Command-line flags are applied first,
...
int main(int argc, char* argv[]) {
    ...
    const std::string path = cflag::args().empty() ? CFLAG_EXAMPLE_DIR "/flags.gflags" : cflag::args()[0];
    cflag::parse_file(path, cflag::flag_file_format::gflags);
